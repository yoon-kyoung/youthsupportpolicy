...
```
main (우리 사이트)  ──●──────────────────────────●── 챗봇 추가 완료!
                     │                          ▲
                     │  복사해서 따로 작업       │  최윤경님 Merge
                     ▼                          │
작업 브랜치           ●───●───●───●── PR 요청 ──┘
                     챗봇  API  관리자
                     개발  연결  페이지
...
**순서:**
1. 임종권이 작업 브랜치에서 챗봇 개발 (main은 안 건드림)
2. 다 되면 **PR 올림** — "합쳐도 될까요?" 요청
3. 팀원분들이 확인하고 괜찮으면 댓글/카톡으로 알려주기
4. **최윤경님이 GitHub에서 "Merge" 버튼 클릭** → main에 합쳐짐
5. 합쳐지면 우리 최종 사이트에 챗봇이 자동 추가됨!
