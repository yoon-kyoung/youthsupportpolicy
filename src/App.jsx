...
function PolicyCard({policy,favIds,onToggle,onGoDetail,compact,delay=0}){
  ...
  return(
    ...
    >
      ...
      {!compact&&<div style={{fontSize:13,color:c.text||"#374151",fontWeight:600}}>{policy.benefit}</div>}
    ...
    <div style={{background:"#f8fafc",minHeight:"100%",animation:"fadeUp 0.25s ease"}}>
      ...
      <div style={{background:c.grad,padding:bp.isDesktop?"52px 40px 44px":bp.isTablet?"36px 24px 30px":"28px 18px 24px",position:"relative",overflow:"hidden",color:"white"}}>
        ...
        <div style={{position:"relative",maxWidth:bp.isDesktop?860:"100%"}}>
          ...
          <div style={{display:"inline-block",background:"rgba(255,255,255,0.18)",border:"1px solid rgba(255,255,255,0.3)",borderRadius:14,padding:bp.isDesktop?"14px 22px":"10px 16px"}}>
            <div style={{fontSize:11,opacity:0.7,marginBottom:4,fontWeight:600,textTransform:"uppercase",letterSpacing:1}}>주요 혜택</div>
            <div style={{fontSize:bp.isDesktop?22:18,fontWeight:900}}>{policy.benefit}</div>
            {policy.amount>0&&<div style={{fontSize:12,opacity:0.75,marginTop:4}}>최대 {policy.amount.toLocaleString()}만원</div>}
          </div>
        </div>
      </div>
